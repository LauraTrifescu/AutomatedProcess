...
BRD Second
    ...
    Click Element   //*[@name="valoare_credit"]
    Clear Element Text      //*[@name="valoare_credit"]
    Press Keys      //*[@name="valoare_credit"]     440000
    Sleep   2s
    BRD Enter Value
    Close Browser
...
Raiffeisen Enter Value for Prima Casa
    ...
    ${PC_RambursTotal}=   Get Text    //div[contains(text(),'Lei')][1]
...
BT Enter Value
    ${DAE_BT}=   Get Text    //*[@class="dae_rez fiftyContent"]
    ${DAE_BT}=   Replace String  ${DAE_BT}    ,   .
    ${DAE_BT}=    Fetch From Left     ${DAE_BT}   %   
    ${DAE_BT}=   Convert to number   ${DAE_BT}
    Set Global Variable     ${DAE_BT}    ${DAE_BT}

    ${Rata_BT}=   Get Text    //*[@class="rata_rez fiftyContent animateInput"]
    ${Rata_BT}=   Replace String  ${Rata_BT}    .   ${EMPTY}
    ${Rata_BT}=    Fetch From Left     ${Rata_BT}   lei
    ${Rata_BT}=   Convert to number   ${Rata_BT}
    Set Global Variable     ${Rata_BT}    ${Rata_BT}
    
    ${Total_BT}=   Get Text    //*[@class="tot_plata_rez fiftyContent animateInput"]
    ${Total_BT}=   Replace String  ${Total_BT}    .   ${EMPTY}
    ${Total_BT}=    Fetch From Left     ${Total_BT}   lei
    ${Total_BT}=   Convert to number   ${Total_BT}
    Set Global Variable     ${Total_BT}    ${Total_BT}
